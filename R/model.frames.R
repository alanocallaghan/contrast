...
model.frame.lme <- function(formula, ..., env = parent.frame()) {
  ...
  ## if necessary
  if (!is.null(correlation)) {
    if (!is.null(corGrpsForm <-
                 getGroupsFormula(correlation, asList = TRUE))) {
        ...
        if (corQ < lmeQ) {
          ...
          attr(correlation, "formula") <-
            eval(parse(text = paste(
              "~",
              deparse(getCovariateFormula(formula(
                correlation
              ))[[2]]),
              "|",
              deparse(groups[[2]])
  ...
  if (inherits(grps, "factor")) {
    ord <-
      order(grps)  #"order" treats a single named argument peculiarly
  } else {
  ...
  if (corQ > lmeQ) {
    ...
    ord <-
      do.call("order", getGroups(dataMix, getGroupsFormula(correlation)))
  }
