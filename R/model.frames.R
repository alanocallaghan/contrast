...
model.frame.gls <- function(formula, ..., env = parent.frame()) {
    ...
    data <- if ("data" %in% names(Call)) {
        eval(Call$data, env)
    } else {
        sys.frame(sys.parent())
    }
    correlation <-
        if ("correlation" %in% names(Call)) {
            eval(Call$correlation, env)
        } else {
            NULL
        }
    weights <-
        if ("weights" %in% names(Call)) {
            eval(Call$weights, env)
        } else {
            NULL
        }
    na.action <-
        if ("na.action" %in% names(Call)) {
            eval(Call$na.action, env)
        } else {
            na.fail
        }
    ...
    mfArgs <- list(
        formula = asOneFormula(formula(glsSt), model, groups),
        data = data,
        na.action = na.action
    )
    ...
    if (!is.null(groups)) {
        groups <- eval(
            parse(text = paste("~1", deparse(groups[[2]]), sep = "|"))
        )
...
model.frame.lme <- function(formula, ..., env = parent.frame()) {
    ...
    random <- if ("random" %in% names(Call)) {
        eval(Call$random, env)
    } else {
        pdSymm(eval(as.call(fixed[-2])))
    }
    correlation <- if ("correlation" %in% names(Call)) {
        eval(Call$correlation, env)
    } else {
        NULL
    }
    weights <- if ("weights" %in% names(Call)) {
        eval(Call$weights, env)
    } else {
        NULL
    }
    method <- if ("method" %in% names(Call)) {
        eval(Call$method, env)
    } else {
        "REML"
    }
    na.action <- if ("na.action" %in% names(Call)) {
        eval(Call$na.action, env)
    } else {
        na.fail
    }
    contrasts <- if ("contrasts" %in% names(Call)) {
        eval(Call$contrasts, env)
    } else {
        NULL
    }
        ...
        if (!is.null(corGrpsForm <- getGroupsFormula(correlation, asList = TRUE))) {
            ...
            if (corQ <= lmeQ) {
                if (any(corGrpsForm != lmeGrpsForm[1:corQ])) {
                    stop(
                        paste(
                            "Incompatible formulas for groups in \"random\"",
                            "and \"correlation\""
                        )
                    )
                }
                if (corQ < lmeQ) {
                    warning(
                        paste(
                            "Cannot use smaller level of grouping for",
                            "\"correlation\" than for \"random\". Replacing",
                            "the former with the latter."
                        )
                    )
                    attr(correlation, "formula") <- eval(
                        parse(text = paste(
                            "~",
    ...
    mfArgs <- list(
        formula = asOneFormula(formula(lmeSt), fixed, groups),
        data = data,
        na.action = na.action
    )
    if ("subset" %in% names(Call)) {
